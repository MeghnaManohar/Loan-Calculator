...
app.layout = dbc.Container(
    [
        ...
        dbc.Row(
            [
                dbc.Col(port_card, md=6),
                dbc.Col(
                    dcc.Graph(id='output-portfolio-chart')
                )
            ],
        align="left",
        ),
...
def generate_table(dataframe, max_rows=25):
    return dash_table.DataTable(
            ...
            style_cell = {'minWidth': 70, 'maxWidth': 85, 'width': 80},
...
@app.callback(
    Output("output-portfolio-chart", "figure"),
    [Input('buttonSearch', 'n_clicks')]
) 
def port_chart(n_clicks):
    if n_clicks < 0:
        return None
    else:
...
def impact2(n_clicks, input1, input2, input3, input4, cont1, cont2, cont3, cont4, cont5):
    if (input1 == None or input2 == None or input3 == None or input4 == None):
        return None
...
# def impact1(n_clicks, input1, input2, input3, input4, cont1, cont2, cont3, cont4, cont5):
#     contributions = [100, 10, 5]
#     person=LoanImpacts(12000.0, 4.0, 70.0, 12.0, contributions) 
#     table=person.compute_impacts()
#     print(type(table))
#     dataframe = pd.DataFrame.from_dict(table, orient='index', columns=['Index', 'InterestPaid', 'Duration', 'MIInterest',
#                    'MIDuration'])
#     cont_table = generate_table(dataframe)
#     print(cont_table)
#     return cont_table
